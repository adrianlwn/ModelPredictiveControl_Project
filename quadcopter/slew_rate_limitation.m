function [ innerController, filter ] = slew_rate_limitation( sys, N, x0, r, T, d )
%SLEW_RATE_LIMITATION Summary of this function goes here
...
%%% SLEW RATE
delta_sr = [ 0.03 ; 0.03 ; 0.03; 0.03]; % slew rate limitation, one per input
...
con = [con, abs(u_prev - du(:,1) + us) <= delta_sr ]

for k = 1:N-1    
    % SLEW RATE CONSTRAINT
    con = [con, abs(du(:,k+1) - du(:,k))  <= delta_sr ]
...
innerController = optimizer(con, obj, ops, [x_init ; ref; u_prev ; d_est_init], du(:,1) + us);
simQuad( sys, innerController, x0, T , r, filter,d, 1);
